...
def _ytdlp_base_args() -> list[str]:
    """
    Base yt-dlp arguments that bypass YouTube bot detection.

    Priority:
    1. cookies.txt file in project dir (most reliable — browser can be open)
    2. YTDLP_BROWSER env var → --cookies-from-browser (browser must be closed)
    3. No cookies (works when IP is not flagged by YouTube)

    Always adds alternative player clients as extra protection.
    """
    args: list[str] = [
        "--extractor-args", "youtube:player_client=tv_embedded,android,web",
    ]

    # 1. cookies.txt file (generated by update-cookies.bat)
    cookies_file = os.path.join(_WORKER_DIR, "cookies.txt")
    if os.path.exists(cookies_file):
        logger.debug("yt-dlp: using cookies.txt")
        args += ["--cookies", cookies_file]
        return args

    # 2. Live browser cookies (fallback — browser must be closed)
